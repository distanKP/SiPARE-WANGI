...
Buat satu Google Spreadsheet baru, lalu buat **3 sheet** (tab) dengan nama dan kolom header persis seperti ini (huruf besar/kecil harus sama):
...
Sheet ini boleh dibiarkan **kosong** (hanya baris header) — hanya terisi kalau ada admin yang mengubah atau di-reset passwordnya. Username dan role dasar tetap ditentukan dari kode aplikasi; sheet ini hanya menyimpan *override* password.

> Catatan: untuk tahap pilot, data referensi (daftar 22 kabupaten, daftar kecamatan, daftar komoditas) **tidak perlu** disimpan di Sheets — data itu sudah tertanam langsung di kode aplikasi dan jarang berubah. Ini menyederhanakan setup dan mengurangi jumlah permintaan ke Apps Script.
>
> Soal sheet `Users`: ini aman meski berisi password, karena sheet ini **tidak perlu dibagikan ke siapa pun** — Apps Script Web App selalu berjalan atas nama akun pemilik (Anda), bukan atas nama pengunjung website. Jadi admin kabupaten tetap tidak bisa melihat isi spreadsheet ini secara langsung, siapa pun mereka.
...
function doGet(e) {
  try {
    ...
    if (action === 'getUsers') {
      return jsonResponse(sheetToObjects(ss.getSheetByName('Users')));
    }
    return jsonResponse({ error: 'Aksi GET tidak dikenal: ' + action });
  } catch (err) {
    // Penting: jangan biarkan error di sini keluar sebagai halaman HTML bawaan
    // Google — selalu balas JSON supaya browser tidak gagal parse ("Unexpected
    // token '<'"). Pesan error tetap muncul jelas di aplikasi.
    return jsonResponse({ error: 'Server error (GET): ' + err.message });
  }
}
...
function doPost(e) {
  ...
  try {
    lock.waitLock(10000); // tunggu maks 10 detik kalau ada penulisan lain yang sedang berjalan
  } catch (lockErr) {
    return jsonResponse({ error: 'Server sibuk (banyak yang menyimpan bersamaan), coba lagi sebentar lagi.' });
  }
  ...
  try {
    ...
    if (body.action === 'setUserPassword') {
      var sheet = ss.getSheetByName('Users');
      var data = sheet.getDataRange().getValues();
      var rowIdx = -1;
      for (var i = 1; i < data.length; i++) {
        if (String(data[i][0]) === String(body.username)) { rowIdx = i + 1; break; }
      }
      if (rowIdx === -1) {
        sheet.appendRow([body.username, body.password]);
      } else {
        sheet.getRange(rowIdx, 2).setValue(body.password);
      }
      return jsonResponse({ success: true });
    }
    ...
    return jsonResponse({ error: 'Server error (POST): ' + err.message });
  } finally {
    lock.releaseLock();
  }
}
...
function normalizeRecord(row) {
  return {
    ...
    tanggal: formatDateField(row.tanggal),
    createdBy: row.dibuat_oleh,
    createdAt: row.dibuat_pada,
  };
}

// PENTING: Google Sheets sering otomatis mengubah teks tanggal ("2026-09-15")
// menjadi tipe Date asli begitu disimpan. Kalau dibiarkan apa adanya, saat
// dibaca kembali nilainya berubah jadi string ISO lengkap dengan jam
// ("2026-09-15T00:00:00.000Z") — ini bikin fitur yang membandingkan tanggal
// persis (Tabulasi → Harian: log input, status kepatuhan, grafik entri per
// hari) gagal cocok setiap kali data dimuat ulang. Fungsi ini memastikan
// tanggal SELALU keluar sebagai teks bersih "yyyy-MM-dd", apa pun tipe aslinya.
...
- **Sudah pernah deploy Apps Script sebelumnya?** Kode di atas menambahkan aksi `updateRecord`, `deleteRecord`, `updateTarget`, `deleteTarget` (untuk tombol edit/hapus di aplikasi), serta `getUsers`/`setUserPassword` (untuk fitur Ubah Password & Reset Password Kabupaten), dan perbaikan format tanggal di `normalizeRecord` (memperbaiki tracking di Tabulasi → Harian yang sebelumnya hilang setelah data dimuat ulang). Kalau Apps Script Anda sudah live sebelum bagian ini ditambahkan, **wajib**: (1) tambahkan sheet baru `Users` di spreadsheet Anda sesuai struktur di bagian 1, (2) buka lagi editor Apps Script, ganti seluruh isi `Code.gs` dengan versi terbaru di atas, lalu **Deploy → Manage deployments → ikon pensil → Version: New version → Deploy** (edit kode saja tidak otomatis update ke URL yang sudah live).
